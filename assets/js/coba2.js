function myFunction() {
    var x = document.getElementById("myTopnav");
    if (x.className === "topnav") {
        x.className += " responsive";
    } else {
        x.className = "topnav";
    }
}

// jam

let nav = document.querySelector("nav"),
    icons = document.querySelector(".icons"),
    silinder = document.querySelector(".silinder"),
    blog_slider = document.querySelector(".blog-slider"),
    topnav = document.querySelector(".topnav"),
    Section = document.querySelector("section"),
    body = document.querySelector("body");


icons.onclick = () => {
    nav.classList.toggle("dark");
    // silinder.classList.toggle("dark");
    // blog_slider.classList.toggle("dark");
    // topnav.classList.toggle("dark");
    // Section.classList.toggle("dark");
    // body.classList.toggle("dark");
}


// creating a function and calling it in every seconds
// setInterval(() => {

//     let date = new Date(),
//         hour = date.getHours(),
//         min = date.getMinutes(),
//         sec = date.getSeconds();

//     let d;
//     d = hour < 12 ? "AM" : "PM"; //if hour is smaller than 12, than its value will be AM else its value will be pm
//     hour = hour > 12 ? hour - 12 : hour; //if hour value is greater than 12 than 12 will subtracted ( by doing this we will get value till 12 not 13,14 or 24 )
//     hour = hour == 0 ? hour = 12 : hour; // if hour value is  0 than it value will be 12
//     // adding 0 to the front of all the value if they will less than 10
//     hour = hour < 10 ? "0" + hour : hour;
//     min = min < 10 ? "0" + min : min;
//     sec = sec < 10 ? "0" + sec : sec;
//     document.querySelector(".hour_num").innerText = hour;
//     document.querySelector(".min_num").innerText = min;
//     document.querySelector(".sec_num").innerText = sec;
//     document.querySelector(".am_pm").innerText = d;

// }, 1000); // 1000 milliseconds = 1s


// silinder

var swiper = new Swiper('.blog-slider', {
    spaceBetween: 30,
    effect: 'fade',
    loop: true,
    mousewheel: {
        invert: false,
    },
    // autoHeight: true,
    pagination: {
        el: '.blog-slider__pagination',
        clickable: true,
    }
});

function mouseOver(over) {
    over.innerHTML = "BACA SELENGKAPNYA";
}

function mouseOut(out) {
    out.innerHTML = "INFORMASI SEKOLAH";
}


let date = new Date();
let year = document.querySelector('.year');



year.innerHTML = date.getFullYear();